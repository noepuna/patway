 ...
 */
	...
	trait TAppFileOps
	{
		...
		public function t_appFileOps_visibilityExists( IConfig $CONFIG, string $VISIBILITY_ID ) : bool
		{
			$db = $CONFIG->getDbAdapter();

			$db->query( "SELECT `uid` FROM `app_file_visibility` WHERE `uid` = :id LIMIT 0, 1", [ 'id' => $VISIBILITY_ID ] );
